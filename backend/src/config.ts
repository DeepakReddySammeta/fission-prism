...
/* Authentication is delegated to the Fission AI Portal gateway, so this
 * server signs no tokens and holds no secret of its own: JWT_SECRET and
 * GOOGLE_CLIENT_ID are gone along with the /api/auth/* routes. What replaces
 * them is a network boundary — see backend/src/auth/auth.ts. */
